...
test_docker_aws_tools_override_is_opt_in_and_ignored_locally() {
  ...
  assert_file_contains "${REPO_ROOT}/docker-compose.aws-instance-profile.yml" 'AWS_EC2_METADATA_DISABLED'
  assert_file_contains "${REPO_ROOT}/.env.docker-aws-tools.example" 'LOCAL_GENAI_LAB_ENABLE_AWS_TOOLS=true'
  assert_file_contains "${REPO_ROOT}/.env.docker-aws-tools.example" 'LOCAL_GENAI_LAB_AWS_CREDENTIAL_SOURCE=host-files'
...
test_docker_start_uses_instance_profile_override_when_configured() {
  local tmp_dir output
  tmp_dir="$(mktemp -d)"
  mkdir -p "${tmp_dir}/bin"
  : >"${tmp_dir}/docker.log"
...
EOF
  ...
  assert_contains "${output}" 'aws tools:'
  assert_contains "${output}" 'enabled with EC2 instance profile credentials'
  assert_file_contains "${tmp_dir}/docker.log" "compose -f ${REPO_ROOT}/docker-compose.yml -f ${REPO_ROOT}/docker-compose.aws-instance-profile.yml up -d --build"
  rm -rf "${tmp_dir}"
}
...
test_docker_aws_preflight_validates_instance_profile_identity() {
  local tmp_dir output
  tmp_dir="$(mktemp -d)"
  mkdir -p "${tmp_dir}/bin"
  : >"${tmp_dir}/docker.log"
...
EOF
  ...
  assert_contains "${output}" 'Docker AWS preflight'
  assert_contains "${output}" 'credential source: instance-profile'
  assert_contains "${output}" 'configured profile: EC2 instance profile'
  assert_contains "${output}" 'checking: backend container is running... pass'
  assert_contains "${output}" 'checking: AWS CLI and jq in backend... pass'
  assert_contains "${output}" 'checking: AWS STS identity... pass'
  assert_contains "${output}" 'Docker AWS preflight passed.'
  if grep -F 'test -d /root/.aws' "${tmp_dir}/docker.log" >/dev/null; then
    printf '%s\n' 'expected instance-profile mode not to check /root/.aws mount' >&2
    exit 1
  fi
  rm -rf "${tmp_dir}"
}
...
EOF
  ...
  assert_contains "${output}" 'Verify the credential source, region settings, IAM access, and EC2 metadata options.'
  rm -rf "${tmp_dir}"
}
