...
5. Tool-assisted chat
   ...
   For S3, verify `Give me a report from AWS S3 for the last month.` enters the S3 tool flow, explains that the current S3 CloudWatch report runs one bucket at a time, and asks for the bucket name. The app should not ask for AWS account ID or username because the tool uses local AWS CLI credentials.
