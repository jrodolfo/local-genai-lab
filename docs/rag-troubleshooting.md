...
## Reset Or Rebuild Qdrant After Embedding Changes

If you change the embedding model, corpus files, chunking settings, or Qdrant
collection name, rebuild the Qdrant index before trusting Qdrant answers. The
normal path is:

1. Select `Vector - Qdrant` in the RAG page.
2. Click `Rebuild Index`.
3. Confirm the status shows a present Qdrant collection with points.

The rebuild operation recreates the configured Qdrant collection and upserts the
current docs chunks with fresh embeddings. That is usually enough; you do not
need to manually delete Docker volumes for normal reindexing.

If the local Qdrant volume is corrupted or you intentionally want to delete all
...
carefully because it removes the local Qdrant volume, not just the running
container.
